...
`nlma.io` becomes tenant `nlma`. Its register moves from static data into these tables.

**Do this now rather than later.** There are currently zero confirmed bookings in the live
register, so the migration is an empty table plus a config row. Every week that passes makes
it a real data migration with real money in flight.
